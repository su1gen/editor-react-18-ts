...
exports.filter = exports.deleteEmptyParagraphAndMoveBlockUp = exports.applyMarkOnRange = void 0;
...
var isEmptySelectionAtStart = function isEmptySelectionAtStart(state) {
  ...
  return empty && ($from.parentOffset === 0 || state.selection instanceof _gapCursorSelection.GapCursorSelection);
};
...
var isEmptySelectionAtEnd = function isEmptySelectionAtEnd(state) {
  ...
  return empty && ($from.end() === $from.pos || state.selection instanceof _gapCursorSelection.GapCursorSelection);
};
...
var isFirstChildOfParent = function isFirstChildOfParent(state) {
  ...
  return $from.depth > 1 ? state.selection instanceof _gapCursorSelection.GapCursorSelection && $from.parentOffset === 0 || $from.index($from.depth - 1) === 0 : true;
};
/**
...
var toggleMark = function toggleMark(markType, attrs) {
  return function (state, dispatch) {
    ...
    if (state.selection instanceof _prosemirrorState.TextSelection && state.selection.$cursor) {
      var tr = state.tr;
...
var deleteEmptyParagraphAndMoveBlockUp = function deleteEmptyParagraphAndMoveBlockUp(canNextNodeMoveUp) {
  return function (state, dispatch, view) {
    ...
    if (empty && nextPMNode && canNextNodeMoveUp(nextPMNode) && (0, _document.isEmptyParagraph)(parent) && view !== null && view !== void 0 && view.endOfTextblock('right')) {

...
function insertSliceAtNodeEdge(_ref2) {
  ...
  var direction = -1;
  tr.setSelection(_prosemirrorState.TextSelection.near(startSlicePosition, direction));
}
...
function insertSliceIntoRangeSelectionInsideList(_ref4) {
  ...
  var newSelection = _prosemirrorState.TextSelection.near(tr.doc.resolve(tr.mapping.map(mapped)), -1);

  newSlice.openEnd = newSlice.openStart;
  tr.replaceRange(newSelection.from, newSelection.from, newSlice);
  tr.setSelection(_prosemirrorState.TextSelection.near(tr.doc.resolve(newSelection.from), -1));
}
...
function insertSliceInsideOfPanelNodeSelected(panelNode) {
  return function (_ref5) {
    ...
    if (panelNode && !panelNode.type.validContent(_prosemirrorModel.Fragment.from(slice.content))) {
      ...
      tr.setSelection(_prosemirrorState.TextSelection.near(tr.doc.resolve(insertPosition + slice.content.size - slice.openStart - slice.openEnd + 1)));
      return;
    }

    var temporaryDoc = new _prosemirrorTransform.Transform(tr.doc.type.createAndFill());
    ...
    var panelNodeSelected = selection instanceof _prosemirrorState.NodeSelection ? selection.node : null;
    ...
    tr.setSelection(_prosemirrorState.TextSelection.near(tr.doc.resolve($from.pos + newPanel.content.size), -1));
  };
}
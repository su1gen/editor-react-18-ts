...
export function formatDateType(date, locale) {
  ...
  const l10n = createLocalizationProvider(locale); // The JS Date api represents month as a number between 0-11 :)
...
export function dateTypeToDate(date) {
  const {
    ...
    month,
    year
  } = date; // The JS Date api represents month as a number between 0-11 :)
  // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date

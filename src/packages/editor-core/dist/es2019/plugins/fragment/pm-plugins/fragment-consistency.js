...
const checkIsAddingSupportedNode = (schema, transaction) => {
  const nodesSupportingFragmentMark = getNodesSupportingFragmentMark(schema);
  return transaction.steps.some(step => nodesSupportingFragmentMark.some(nodeType => stepAddsOneOf(step, new Set([nodeType]))));
};
...
const regenerateFragmentIdIfNeeded = ({
  ...
  doc.descendants((node, pos) => {
    const isSupportedNode = nodesSupportingFragmentMark.some(supportedNode => node.type === supportedNode);
    ...
    if (shouldRegenerateLocalId(existingFragmentMark.attrs.localId)) {
      ...
      transaction.setNodeMarkup(pos, undefined, node.attrs, node.marks.map(mark => {
...
export const createPlugin = dispatch => new SafePlugin({
  ...
  appendTransaction: (transactions, _oldState, newState) => {
    ...
    transactions.forEach(transaction => {
      const fragmentLocalIdsObserved = new Set();
      ...
      const isAddingSupportedNode = checkIsAddingSupportedNode(newState.schema, transaction);
      ...
      const {
        ...
        transaction: tr,
        shouldRegenerateLocalId: localId => {

...
export function handlePasteLinkOnSelectedText(slice) {
  return (state, dispatch) => {
    ...
    let linkMark; // check if we have a link on the clipboard
    ...
    if (slice.content.childCount === 1 && isParagraph(slice.content.child(0), schema)) {
      ...
      if (paragraph.content.childCount === 1 && isText(paragraph.content.child(0), schema)) {
        const text = paragraph.content.child(0); // If pasteType is plain text, then
...
export function handleParagraphBlockMarks(state, slice) {
  ...
  } = state; // If no paragraph in the slice contains marks, there's no need for special handling
...
export const handleSelectedTable = slice => (state, dispatch) => {
  let tr = replaceSelectedTable(state, slice); // add analytics after replacing selected table

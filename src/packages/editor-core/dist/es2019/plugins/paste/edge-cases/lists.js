...
export function insertSliceIntoRangeSelectionInsideList({
  ...
  } = tr; // when the selection is inside of the same list item
...
export function insertSliceInsideOfPanelNodeSelected(panelNode) {
  return ({
    ...
    const {
      from: panelPosition
    } = selection; // if content of slice isn't valid for a panel node, insert the invalid node and following content after

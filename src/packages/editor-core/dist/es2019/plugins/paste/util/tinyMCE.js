...
const getTableElementsInfo = doc => {
  ...
  const hasCompleteRow = // we take header-only and cell-only tables to be
  // row-complete
...
export const tryRebuildCompleteTableHtml = incompleteTableHtml => {
  ...
  const isExpectedStructure = tableWrappedDoc.querySelectorAll('body > table:only-child') && !tableWrappedDoc.querySelector(`body > table > tbody > tr > :not(th,td)`); // if DOMParser saves table elements that we would otherwise lose, and

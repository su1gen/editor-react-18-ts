...
export function transformToCodeBlockAction(state, start, attrs) {
  ...
  const startLinePosition = startOfCodeBlockText.start(); //when cmd+A is used to select the content. start position should be 0.
  ...
  const codeBlockSlice = mapSlice(contentSlice, (node, parent, index) => {
    ...
    const isParentLastChild = parent && contentSlice.content.childCount - 1 === index; // add line breaks at the end of each paragraph to mimic layout of selected content
  ...
  const tr = state.tr; // Replace current block node

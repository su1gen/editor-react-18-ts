...
function createSingleQuotesRules() {
  ...
  createRule(/(\s|^)'(\S+.*\S+)'$/, (state, match, start, end) => {
    ...
    const [, spacing, innerContent] = match; // Edge case where match begins with some spacing. We need to add
    // it back to the document

    const openQuoteReplacement = spacing + OPEN_SMART_QUOTE_CHAR; // End is not always where the closed quote is, edge case exists
...
export default (featureFlags => createPlugin('text-formatting:smart-input', [...getProductRules(), ...getSymbolRules(), ...getPunctuationRules()]));
...
export function createLinkInputRule(regexp, skipAnalytics = false) {
  ...
  return createRule(regexp, (state, match, start, end) => {
    ...
    const filepaths = findFilepaths(nodeBefore.text, // The position referenced by 'start' is relative to the start of the document, findFilepaths deals with index in a node only.
...
export function createInputRulePlugin(schema, skipAnalytics = false, featureFlags) {
  ...
  const urlWithASpaceRule = createLinkInputRule(LinkMatcher.create(), skipAnalytics); // [something](link) should convert to a hyperlink

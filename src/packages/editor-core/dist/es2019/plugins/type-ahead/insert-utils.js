...
function findInsertPoint(doc, pos, nodeToInsert) {
  ...
  for (let level = $pos.depth; level >= 0; level--) {
    const nodeAtLevel = $pos.node(level); // Try to replace the empty paragraph in the level above
    // Scenario:
    //   doc(
    //     table(
    //       row(
    ...
    const parentNode = $pos.node(levelAbove); // Special case: when this is true, the 'to' position should be the end
...
export const insertBlockNode = ({
  ...
  if (node.isBlock) {
    ...
    const nodeNormalized = normaliseNestedLayout(tr, node); // Handle edge cases for hr and mediaSingle
